...
### Fixed
- Fixed `NoSuchFileException` when running `bal migen module --package <org/name>` against a Ballerina Central package. The JAR copy step assumed the emitted artifact always resided under `target/mi/bin/`, which is only true for local `BuildProject` source. A Central package loads as a `BalaProject` and the compiler emits the JAR to `target/mi/BalConnectors/` instead. Both `ConnectorSerializer` and `ResourcePackager` now use the `sourcePath` reference (already set to the correct emitted JAR path by `compileAnalyzeAndEmit`) rather than recomputing a hardcoded `bin/` path.
