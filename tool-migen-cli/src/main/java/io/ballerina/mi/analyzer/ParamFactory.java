        ...
        for (TypeSymbol memberTypeSymbol : unionTypeSymbol.memberTypeDescriptors()) {
            ...
            if (paramType != null) {
                if (TypeDescKind.NIL.getName().equals(paramType)) {
                    ...
                    if (actualTypeKind == TypeDescKind.UNION && actualMemberTypeSymbol instanceof UnionTypeSymbol memberUnionSymbol) {
                    ...
                    } else if (actualTypeKind == TypeDescKind.MAP && actualMemberTypeSymbol instanceof MapTypeSymbol mapTypeSymbol) {
                        MapFunctionParam mapFieldParam = new MapFunctionParam(Integer.toString(memberIndex), memberParamName, paramType);
                        mapFieldParam.setTypeSymbol(memberTypeSymbol);
                        mapFieldParam.setDisplayTypeName(actualParamType);
                        mapFieldParam.setRequired(functionParam.isRequired());
                        ...
                        if (valueTypeOpt.isPresent()) {
                            ...
                            if (shouldRender && valueTypeKind == TypeDescKind.RECORD) {
                                TypeSymbol actualValueType = Utils.getActualTypeSymbol(valueType);
                                if (actualValueType instanceof RecordTypeSymbol valueRecordType) {
                                    populateMapValueFields(mapFieldParam, valueRecordType);
                                }
                            }
                        }
                        memberParam = mapFieldParam;
                    } else if (actualTypeKind == TypeDescKind.ARRAY && actualMemberTypeSymbol instanceof ArrayTypeSymbol arrayTypeSymbol) {
                        ArrayFunctionParam arrayFieldParam = new ArrayFunctionParam(Integer.toString(memberIndex), memberParamName, paramType);
                        arrayFieldParam.setTypeSymbol(memberTypeSymbol);
                        arrayFieldParam.setDisplayTypeName(actualParamType);
                        arrayFieldParam.setRequired(functionParam.isRequired());
                        ...
                        if (shouldRender && elementTypeKind == TypeDescKind.RECORD) {
                            TypeSymbol actualElementType = Utils.getActualTypeSymbol(elementType);
                            if (actualElementType instanceof RecordTypeSymbol elementRecordType) {
                                populateArrayElementFields(arrayFieldParam, elementRecordType);
                                arrayFieldParam.setSupportsDualInputMode(true);
                            }
                        }
                        if (shouldRender && elementTypeKind == TypeDescKind.ARRAY) {
